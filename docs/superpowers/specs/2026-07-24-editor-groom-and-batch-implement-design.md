...
| Discarded rows | **Checkbox disabled** | `master-backlog-executor`'s pre-flight declines the *entire batch* if any id is discarded; one slipped-in id would waste a whole run. Shipped rows stay selectable — the gate merely skips them. |
