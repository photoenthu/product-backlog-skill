...
**Artifact links** resolve two ways: a full `http(s)://` URL opens externally in a new tab; a **relative in-repo path** (e.g. `docs/superpowers/plans/foo.md`) is served by the editor's server from the **project root** (git toplevel of the backlog, else the repo layout's root) via a `/file` route, so it opens in the browser. That route is confined to the project root (path traversal is rejected) and serves HTML/SVG/XML as inert `text/plain` so an artifact can't script the editor's origin. Filter/sort/search state also persists across page refreshes (per-backlog, in `localStorage`).
